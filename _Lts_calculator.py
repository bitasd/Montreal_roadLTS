...
class Mixin:
    ...
    # TODO: change ID_CYCLABLE logic
    # TODO:
    def calc_lts(self, m_id_cycl, m_gestion, spd_q85, lane_p_direction, one_way, adt,
                 centerline, cyclway_attr):
        mixed_traffic = 0
        on_street_bikelane = 0
        protected_path = 0

        if pandas.isna(m_id_cycl):  # empty ID_CYCL -- coming from geobase layer(on-street and mixed traffic) # no bike facility
            if 'type_voie' in cyclway_attr.keys():  # 1. road w/ a bike lane  2. bikelane and a nearby path 3. mixed traffic with a nearby off-street path --use gestion
                if cyclway_attr['type_voie'] in (3, 9):
                    if cyclway_attr['type_voie2'] in (30, 31) or cyclway_attr['ed_typvoi2'] in (31, 30):  # contraflow bikelane
                ...
                else:  # 0, -9999 city has not got around categorizing the type_voie -- could be anything
            ...
            if cyclway_attr['drvewayVis'] == 0:
                _lts = 1
            else:
                _lts = 2

        # Protected and paths
        if 'Prot_posD' in cyclway_attr.keys() and re.search('yes', str(cyclway_attr['Prot_posD']), re.IGNORECASE):
            _lts = 1
        ...
        if on_street_bikelane == 1:
            if float(cyclway_attr['BLanW_posD']) > 0:

                # TABLE 2
                if ('ParkW_posD' in cyclway_attr.keys()) and (float(cyclway_attr['ParkW_posD']) > 0):
                    if lane_p_direction == 1:
                        if float(cyclway_attr['Reach_posD']) >= 4.5:
                            if float(spd_q85) <= 46:
                                _lts = 1
                                ...
                                _lts = 'c1'
                        elif 4.4 >= float(cyclway_attr['Reach_posD']) >= 3.5:
                            if spd_q85 <= 54:
                                _lts = 2
                            elif spd_q85 <= 62:
                                _lts = 3
                        ...
                        elif float(cyclway_attr['Reach_posD']) < 4.5:
                            _lts = 3
                        else:
                            _lts = 3
                    ...
                    if lane_p_direction == 1 or centerline == 0:
                        if 'Reach_posD' in cyclway_attr.keys():
                            if float(cyclway_attr['Reach_posD']) >= 1.8:
                                if spd_q85 <= 54:
                                    _lts = 1
                                elif spd_q85 <= 62 and spd_q85 > 54:
                                    _lts = 2
                                elif spd_q85 > 62:
                                    _lts = 3
                                else:
                                    _lts = 'c8'
                            elif float(cyclway_attr['Reach_posD']) < 1.8:
                                if spd_q85 <= 62:
                                    _lts = 2
                                elif 78 >= spd_q85 > 62:
                                    _lts = 3
